...
- Источники серверов в `targets.txt`: поддержка `host[:port]`, `ip[:port]`, `CIDR`, а также диапазонов портов (`25565-25590`).
- Автоперезагрузка списка целей на каждом цикле (можно менять `targets.txt` без рестарта).
- Если сервер временно недоступен, в сумме используется его последнее успешно полученное значение онлайна (до следующего успешного пинга).
...
192.168.10.20:25566
144.31.225.0/24
mc.example.net:25565-25590
144.31.225.0/24:25565-25570
```

> Для CIDR по умолчанию используется порт `25565`, но можно указать и диапазон портов через `:start-end`.
